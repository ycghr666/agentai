	...
	#createAfterModelSequenceRouter(toolClasses, allowed, nextDefault, hasToolsAvailable = toolClasses.length > 0) {
		...
		return (state) => {
			...
			if (builtInState.jumpTo) {
				...
				if (dest === require_AgentNode.AGENT_NODE_NAME && allowedSet.has(require_AgentNode.AGENT_NODE_NAME)) return new _langchain_langgraph.Send(require_AgentNode.AGENT_NODE_NAME, {
					...state,
					jumpTo: void 0
				});
			}
