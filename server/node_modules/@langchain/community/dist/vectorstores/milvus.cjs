...
var Milvus = class Milvus extends _langchain_core_vectorstores.VectorStore {
	...
	*/
	async similaritySearchVectorWithScore(query, k, filter) {
		...
		if (hasColResp.status.error_code !== _zilliz_milvus2_sdk_node.ErrorCode.SUCCESS) throw new Error(`Error checking collection: ${hasColResp}`);
	...
	*/
	async createCollection(vectors, documents) {
		...
		if (createRes.error_code !== _zilliz_milvus2_sdk_node.ErrorCode.SUCCESS) throw new Error(`Failed to create collection: ${createRes}`);
		const extraParams = {
			...this.indexCreateParams,
